        ...
        <section class="section daftar" id="daftar">
            <div class="row">
                <div class="col">
                    <div class="card">
                        <div class="card-body">
                            <h2 class="card-title">Pencarian berdasarkan kata</h2>

                            <h6>Results for: <strong>"{{ $keyword }}"</strong></h6>

                            @foreach ($results as $result)
                                @php
                                    $highlightedArticle = preg_replace(
                                        '/(' . preg_quote($keyword, '/') . ')/i',
                                        "<span class='highlight'>$1</span>",
                                        $result['artikel']->artikel,
                                    );
                                @endphp
                                <p style="text-align: justify;">{!! $highlightedArticle !!}</p>
                                <p><strong>Count: {{ $result['count'] }}</strong></p>
                            @endforeach
